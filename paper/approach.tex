...
However, we argue that presenting replicated datatypes in this manner has a number of significant advantages:
\begin{enumerate}
\item
Unlike typical definitions of CRDT algorithms \cite{Shapiro:2011wy,Shapiro:2011un}, it is not necessary for $\mathrm{interp}\big[S,\, (\mathit{id},\, \mathit{op})\big]$ to commute with respect to other operations: any pure function can be used.
...
\item
Nodes retaining an explicit set of observed operations provides a simple mechanism for recovering from network partitions and failures: missing operations may be retransmitted and added to the OpSets of previously partitioned nodes.
Similarly, when a new node joins the network, it can be initialised with the union of OpSets of existing nodes.
\item
Our executable specifications guarantee the existence of an implementation of the described datatype, unlike axiomatic specifications, which may not be implementable.
%They are witnesses of their own specification.
\end{enumerate}
