...
Our datatypes are also composable, enabling the construction of complex data structures such as a list of maps.
...
In particular, we highlight an important new correctness property for collaborative text editing, which has traditionally been overlooked.
Algorithms that do not satisfy this property can exhibit awkward interleaving of text.
We use OpSets to specify the required correctness property and prove that one existing algorithm satisfies this property, while other published algorithms do not.
Finally, we also show how OpSets can be used to develop new replicated datatypes, by providing a simple specification of an atomic move operation for trees, an operation that had previously been thought to be impossible to implement without locking.
