...
In this work we have introduced Operation Sets (or OpSets for short), a novel approach for specifying the semantics of replicated datatypes, and reasoning about algorithms for concurrent data access and manipulation, and using our abstraction we have specified a variety of abstract replicated datatypes.
Our use of mechanised proof removes any potential ambiguity in our specifications and eliminates reasoning errors.
...
We have proved that the RGA algorithm satisfies our list specification; as a CRDT, RGA allows operations to be applied in any causal order while our specification defines the interpretation of operations in strictly ascending order of ID.
For future work it will be interesting to explore whether other datatype specifications can be translated into commutative CRDT implementations.

Finally, one can regard the OpSet as a \emph{database of facts}, containing all changes ever made to the shared data, and the interpretation function a \emph{query} over this database, with the resulting datatype a \emph{materialized view} in database terminology.
