...
\begin{algorithmic}
    ...
    \State $\mathit{prev} = \{ \mathit{id} \mid \exists\,\mathit{val}.\; (\mathit{id}, \mathit{map}, \mathit{key}, \mathit{val}) \in E \}$
...
\begin{algorithmic}
    ...
    \State $\mathit{prev} = \{ \mathit{id} \mid \exists\,\mathit{val}.\; (\mathit{id}, \mathit{list}, \mathit{key}, \mathit{val}) \in E \}$
...
\begin{align*}
    \big\llbracket (\mathit{id},\, \mathsf{Assign}(\mathit{obj}, \mathit{key}, \mathit{val}, \mathit{prev})) \big\rrbracket_{E,\, L} \;=\; \Big( &
    ...
    %%%%%%%%%%
    \big\llbracket (\mathit{id},\, \mathsf{Remove}(\mathit{obj}, \mathit{key}, \mathit{prev})) \big\rrbracket_{E,\, L} \;=\; \Big( &
    \big\{ (\mathit{id}', \mathit{obj}', \mathit{key}', \mathit{val}') \in E \mid
    \mathit{id}' \notin \mathit{prev} \big\},\; L \Big) \\[5pt]
    %%%%%%%%%%
    \big\llbracket (\mathit{id},\, \mathsf{InsertAfter}(\mathit{ref})) \big\rrbracket_{E,\, L} \;=\; \Big( & E,\;
    \big\{ (p,n) \in L \mid p \neq \mathit{ref} \big\} \;\cup\;
    \big\{ (\mathit{ref}, \mathit{id}) \big\} \;\cup\;
    \big\{ (\mathit{id}, n) \mid (\mathit{ref}, n) \in L \big\} \Big) \\[10pt]
    %%%%%%%%%%
    \big\llbracket (\mathit{id},\, \mathsf{MakeVal}(\mathit{val})) \big\rrbracket_{E,\, L} \;=\; (E,\, L) \qquad
    \big\llbracket&(\mathit{id},\, \mathsf{MakeMap}) \big\rrbracket_{E,\, L} \;=\; (E,\, L) \qquad
    \big\llbracket (\mathit{id},\, \mathsf{MakeList}) \big\rrbracket_{E,\, L} \;=\; (E,\, L)
\end{align*}

The interpretation of $\mathsf{Assign}$ and $\mathsf{Remove}$ updates only $E$ and leaves $L$ unchanged; conversely, the interpretation of $\mathsf{InsertAfter}$ updates only $L$.
...
Thus, from a user's point of view a list element only exists if it has at least one associated value in the $E$ relation; any list elements without associated value should be ignored.
