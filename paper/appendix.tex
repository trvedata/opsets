...
We prove the absence of interleaving for three list elements \isa{xid}, \isa{yid}, and \isa{zid}, where \isa{xid} and \isa{zid} are both inserted after the same reference element, and \isa{yid} is inserted after \isa{xid}.
We then show that, regardless of the order in which these operations are applied, \isa{zid} is never inserted between \isa{xid} and \isa{yid}.
\begin{isabelle}
\isacommand{theorem}\isamarkupfalse%
...
\ \ \ \ \ \ \ \ \ {\isacharparenleft}xid\ {\isasymnotin}\ set\ {\isacharparenleft}interp{\isacharunderscore}ins\ ops{\isacharparenright}\ {\isasymand}\ yid\ {\isasymnotin}\ set\ {\isacharparenleft}interp{\isacharunderscore}ins\ ops{\isacharparenright}\ {\isasymand}\isanewline
\ \ \ \ \ \ \ \ \ \ zid\ {\isasymnotin}\ set\ {\isacharparenleft}interp{\isacharunderscore}ins\ ops{\isacharparenright}{\isacharparenright}{\isachardoublequoteclose}\isanewline
\end{isabelle}
