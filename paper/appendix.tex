\section{Generating Operations}\label{sect:appendix:generating-ops}
...
\begin{algorithmic}[0]
    ...
    \Function{insListIndex}{$O, \mathit{list}, \mathit{index}, \mathit{val}$}
    \State $(E,\, L) = \llbracket O \rrbracket$
    \State $(\mathit{id}_1, \mathit{op}_1) = \Call{valueID}{O, \mathit{val}}$
    \If{$\mathit{op}_1 \neq \bot$}
    \State $O := O \;\cup\; \big\{ (\mathit{id}_1, \mathit{op}_1) \big\})$
...
The list manipulation functions \textsc{setListIndex}, \textsc{insListIndex} and \textsc{removeListIndex} take a numeric index as argument to identify the position in the list being edited.
...
\subsection{No Interleaving}\label{appendix:no-interleaving}
...
\ \ \isakeyword{shows}\ {\isachardoublequoteopen}a\ {\isasymin}\ dom\ {\isacharparenleft}vals{\isacharparenright}\ {\isasymlongleftrightarrow}\ {\isacharparenleft}{\isasymexists}ref\ val{\isachardot}\ {\isacharparenleft}a{\isacharcomma}\ Insert\ ref\ val{\isacharparenright}\ {\isasymin}\ set\ ops{\isacharparenright}\ {\isasymand}\isanewline
