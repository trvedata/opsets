...
The \emph{ancestor} relation is the transitive closure of the parent relation, which we can define using the value relation $V$:
\begin{align*}
    \mathrm{parent}(V,\, i) &=
    \begin{cases}
        \big\{ (\mathit{obj}, \mathit{val}) \mid \exists\,\mathit{id}, \mathit{key}.\;
            (\mathit{id}, \mathit{obj}, \mathit{key}, \mathit{val}) \in V \big\} & \text{if } i=1 \\
        \big\{ (x, z) \mid (x, y) \in \mathrm{parent}(V,\, i-1) \;\wedge\;
            (y, z) \in \mathrm{parent}(V,\, 1) \big\} & \text{if } i > 1
    \end{cases} \\[8pt]
    \mathrm{ancestor}(V) &= \bigcup_{i \;\geq\; 1} \mathrm{parent}(V,\, i)
\end{align*}
...
\[
    \big\llbracket (\mathit{id},\, \mathsf{Assign}(\mathit{obj}, \mathit{key}, \mathit{val}, \mathit{prev})) \big\rrbracket_{V,\, L} \;=\; \left\{
    \arraycolsep=0pt \def\arraystretch{1.5}
    \begin{array}{llllll}
        \multicolumn{2}{l}{(V,\, L)}
        & \text{if } (\mathit{val},\, \mathit{obj}) \in \mathrm{ancestor}(V) \\[8pt]
        \Big( & \multicolumn{2}{l}{
...
Secondly, any existing tuple in $V$ that references the same value $\mathit{val}$ is removed, preserving the invariant that every non-root node must have exactly one parent.
